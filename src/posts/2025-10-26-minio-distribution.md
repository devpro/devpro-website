...
This change, detailed in [GitHub issue #21647](https://github.com/minio/minio/issues/21647#issuecomment-3418675115), means no more pre-built Docker images or binaries from official repositories, impacting users relying on automated updates.
...
This pivot reflects MinIO Inc.'s focus on its enterprise 'AIStor' offering, providing 5-year LTS for paying customers via SUBNET, while the community edition receives only bug fixes and CVE patches.
...
The latest official MinIO image is outdated, harboring CVE-2025-62506, where restricted accounts can spawn unrestricted ones, risking unauthorized data access.

No patches are available for legacy images, impacting auto-updating setups such as homelabs and CI/CD pipelines.
Enterprise users get binaries through AIStor, but the community must build manually, posing challenges for non-developers and small deployments.

![Docker Hub Search for MinIO](images/dockerhub-minio-20251026.png)

This screenshot from Docker Hub (October 26, 2025) displays the official `minio/minio` image with over 1 billion pulls, last updated approximately two months ago, alongside the now-obsolete Bitnami alternative (50M+ pulls, updated about one month ago).

## Available distributions for the community

For pre-built images, community options include:

1. [`cgr.dev/chainguard/minio`](https://images.chainguard.dev/directory/image/minio/versions) by [Chainguard](https://www.chainguard.dev/)

    - 🟢 0 CVE
    - 🟢 Small size (58 MB)
    - 🔴 Only latest

2. [`docker.io/elestio/minio`](https://hub.docker.com/r/elestio/minio) by [elestio](https://elest.io/)

    - 🟢 Small size (60 MB)
    - 🔴 Only latest
    - 🔴 1 HIGH CVE (CVE-2025-62506)

3. [`docker.io/rapidfort/minio-ib`](https://hub.docker.com/r/rapidfort/minio-ib) by [RapidFort](https://www.rapidfort.com/)

    - 🟢 Many versions
    - 🟢 Smallest size (45 MB)
    - 🔴 Most recent push is 5 months ago

4. [`docker.io/elasticio/minio`](https://hub.docker.com/r/elasticio/minio) by [elastic.io](https://www.elastic.io/)
...
- **Why avoid `:latest` in Enterprises/Production?**: The `:latest` tag is a significant red flag for production environments because it points to the most recent image build, which may include untested updates, breaking changes, or unpatched vulnerabilities during rollout.
Without version pinning, automatic updates (e.g., via Watchtower) risk downtime, data corruption, or security gaps, violating enterprise SLAs, compliance requirements (e.g., ISO 27001, PCI DSS), and best practices for reproducible deployments.
...
- [Chainguard](https://images.chainguard.dev/directory/image/minio/overview)
